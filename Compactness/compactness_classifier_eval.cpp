...
int main(int argc, char** argv)
{
    ...
    for (std::size_t i = 0; i < generator.number_of_scales(); ++i) {
        ...
        if (COMPUTE_COMPACTNESS)
            features.add_with_scale_id<MyFeature>(i, pts, pts.point_map(), grid, generator.radius_neighbors(i), wrap); //désactiver la classification
    }
    ...
    std::ifstream in_config("ethz_random_forest.bin", std::ios_base::binary);
    ...
    // Write result
    std::ofstream f("load_classification_ethz_random_forest.ply");
    f.precision(18);
    f << pts;
